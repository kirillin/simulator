...
class PhysicsEngine:
    ...
    def update(self, objects, surface, dt):
        for obj in objects:
            ...
            F_rolling = -0.1 * obj.mass * self.gravity[1] * np.sign(obj.v[0])
            F = Fg + F_rolling
            dv = F / obj.mass

            M = -0.1 * np.sign(obj.omega) * obj.omega
            ...
            obj = self._collision_resolve(obj, surface)

    def _collision_resolve(self, obj, surface):
        ...
        if penetration > 0:
            ...
            mu = 0.1
            v_ct = np.dot(obj.v, t) + obj.omega * obj.radius
            Jt = -mu * Jn * np.sign(v_ct)
            
            obj.v += (Jn * n + Jt * t) / obj.mass
            obj.omega += obj.radius * Jt / obj.inertia
...
def main():
    ...
    # surface = Surface(type="line", k=0.707, b=0.0)  # kx + b
    ...
    for y0 in map(float, range(-200, 300, 25)):
        for x0 in map(float, range(-200, 300, 25)):
